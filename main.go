...
func usage() {
	fmt.Fprintf(os.Stderr, "usage: benchstatn [options] old.txt [new.txt] [more.txt ...]\n")
...
var (
	...
	flagShowDelta  = flag.Bool("showdelta", false, "show delta differences if comparing more than two files")
	flagDeltaWorst = flag.Bool("deltaworst", false, "show delta differences against worst case instead of best case")
)
...
func main() {
	...
	c := &benchstat.Collection{
		...
		DeltaTest:  deltaTest,
		ShowDelta:  *flagShowDelta,
		DeltaWorst: *flagDeltaWorst,
	}
...
td.better { font-weight: bold; color: green; }
td.same { font-weight: bold; color: blue; }
td.worse { font-weight: bold; color: red; }
